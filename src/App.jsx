...
const MATERIAL_LOT_MIGRATION_VERSION_KEY = 'materialLotMigrationVersion'
const MATERIAL_LOT_MIGRATION_BACKUP_KEY = 'materialLotMigrationBackupV1'
const MATERIAL_LOT_MIGRATION_VERSION = 1
...
const RAW_MATERIALS_LEGACY_LABEL = ['Kho', 'nguyên liệu'].join(' ')
const FINISHED_GOODS_LEGACY_STAGE = ['Kho', 'thành phẩm'].join(' ')
const FINISHED_GOODS_LEGACY_ACTION = ['Nhập kho', 'thành phẩm'].join(' ')
const FINISHED_GOODS_WAITING_STATUS = ['Chờ nhập kho', 'thành phẩm'].join(' ')
...
const normalizeMaterialLotCode = (value = '') => String(value || '').trim().replace(/^LOT-/i, '')

function migrateMaterialLotReferences(value, key = '') {
  if (Array.isArray(value)) return value.map((item) => migrateMaterialLotReferences(item, key))
  if (value && typeof value === 'object') return Object.fromEntries(Object.entries(value).map(([childKey, childValue]) => [childKey, migrateMaterialLotReferences(childValue, childKey)]))
  const referenceKeys = ['internallotcode', 'materiallotcode', 'rawmateriallotcode']
  return typeof value === 'string' && referenceKeys.includes(String(key).toLowerCase()) ? normalizeMaterialLotCode(value) : value
}

function runMaterialLotMigration() {
  if (typeof localStorage === 'undefined' || Number(localStorage.getItem(MATERIAL_LOT_MIGRATION_VERSION_KEY) || 0) >= MATERIAL_LOT_MIGRATION_VERSION) return
  const keys = [DATA_KEY, MATERIAL_LOTS_KEY, MATERIAL_LOT_USAGES_KEY, MATERIAL_TRACE_ADJUSTMENTS_KEY, PRODUCTION_ORDERS_KEY, PRODUCTION_LOGS_KEY, QC2_LOGS_KEY, SUPPLEMENTAL_WEIGHING_KEY, WEIGHED_CONTAINERS_KEY]
  const backup = {}
  keys.forEach((storageKey) => {
    const raw = localStorage.getItem(storageKey)
    if (raw == null) return
    backup[storageKey] = raw
  })
  if (!safeSetJsonLocalStorage(MATERIAL_LOT_MIGRATION_BACKUP_KEY, { createdAt: new Date().toISOString(), data: backup })) return
  keys.forEach((storageKey) => {
    const raw = backup[storageKey]
    if (raw == null) return
    try { localStorage.setItem(storageKey, JSON.stringify(migrateMaterialLotReferences(JSON.parse(raw)))) } catch (error) { console.warn(`Không thể migration ${storageKey}.`, error) }
  })
  safeSetLocalStorage(MATERIAL_LOT_MIGRATION_VERSION_KEY, String(MATERIAL_LOT_MIGRATION_VERSION))
}
      ...
      const message = page === 'chemical'
        ...
        : page === 'mixing'
          ...
          : page === 'finished-qc'
            ...
            : page === 'finished-goods'
              ? 'Chưa có lệnh chờ ghi nhận thành phẩm.'
...
function stripDeprecatedScaleToolData(source = {}) {
  const next = { ...(source || {}) }
  delete next[['weighing', 'Tool', 'Catalog'].join('')]
  return next
}
...
function buildRawMaterialLotQr(lot) {
  return normalizeMaterialLotCode(lot.lotCode)
}
...
const buildInternalLotCode = (materialCode = '', receiptDate = todayText(), lots = [], sequenceOverride = null) => {
  ...
  const usedSequences = (lots || []).map((lot) => normalizeMaterialLotCode(lot.internalLotCode || lot.lotCode)).filter((lotCode) => lotCode.startsWith(`${prefix}-`)).map((lotCode) => num(lotCode.slice(prefix.length + 1)))
...
function normalizeMaterialLot(item = {}) {
  ...
  const lot = {
    ...item,
    id: item.id || uid('MLOT'),
    internalLotCode: normalizeMaterialLotCode(item.internalLotCode || item.lotCode || ''),
...
function normalizeMaterialLotUsage(item = {}) {
  return {
    ...
    internalLotCode: normalizeMaterialLotCode(item.internalLotCode || item.materialLotCode || ''),
...
function validateRawMaterialQr(qrInput, requiredMaterial, lots = []) {
  const scannedLotCode = normalizeMaterialLotCode(qrInput)
  const normalizedLots = normalizeMaterialLots(lots)
  const scannedLot = normalizedLots.find((lot) => normalizeCode(lot.internalLotCode) === normalizeCode(scannedLotCode))
  if (!scannedLot) return { ok: false, message: `Mã lô NL ${scannedLotCode || '-'} không tồn tại.` }
  if (normalizeCode(scannedLot.materialCode) !== normalizeCode(requiredMaterial.materialCode)) return { ok: false, message: `Sai mã vật tư. Yêu cầu: ${requiredMaterial.materialCode}; lô đã quét: ${scannedLot.materialCode}.`, lot: scannedLot }
  if (scannedLot.status !== 'active') return { ok: false, message: `Mã lô NL ${scannedLot.internalLotCode} không ở trạng thái active (${scannedLot.status}).`, lot: scannedLot }
  if (num(scannedLot.traceBalanceQty) <= 0) return { ok: false, message: `Mã lô NL ${scannedLot.internalLotCode} đã hết số dư truy xuất.`, lot: scannedLot }
  if (scannedLot.expiryDate && String(scannedLot.expiryDate).slice(0, 10) < todayText()) return { ok: false, message: `Mã lô NL ${scannedLot.internalLotCode} đã hết hạn sử dụng.`, lot: scannedLot }
  const fifoLot = normalizedLots.filter((lot) => normalizeCode(lot.materialCode) === normalizeCode(requiredMaterial.materialCode) && lot.status === 'active' && num(lot.traceBalanceQty) > 0).sort((a, b) => String(a.bravoReceiptDate).localeCompare(String(b.bravoReceiptDate)) || String(a.createdAt).localeCompare(String(b.createdAt)))[0]
  if (fifoLot && fifoLot.internalLotCode !== scannedLot.internalLotCode) return { ok: false, message: `Sai FIFO. Lô ưu tiên: ${fifoLot.internalLotCode}; lô đã quét: ${scannedLot.internalLotCode}.`, lot: scannedLot, fifoLot }
  return { ok: true, message: 'Đúng Mã lô NL, đúng vật tư, đúng FIFO. Cho phép cân.', lot: scannedLot, qr: { lotCode: scannedLot.internalLotCode, materialCode: scannedLot.materialCode }, materialCheck: { scannedCode: scannedLot.materialCode } }
...
const productionAssignmentStages = ['Cân hóa', 'Cân rắn', 'Phối trộn', 'QC sản xuất thử', 'QC thành phẩm', 'Đóng gói', FINISHED_GOODS_LEGACY_STAGE]
...
const productionProcessOptions = [
  ...
  ['PACKAGING', 'Đóng gói'],
  ['FINISHED_GOODS', FINISHED_GOODS_LEGACY_STAGE],
]
...
const productionEmployeeCatalog = [
  ...
  ['NV021', 'VÕ THỊ LAN', 'Tổ phối trộn 2', 'Thủ kho', FINISHED_GOODS_LEGACY_ACTION],
...
const normalizeEmployeeCatalogData = (employees = productionEmployeeCatalog) => (Array.isArray(employees) ? employees : []).map((employee) => {
  ...
  return {
    ...
    operationRole: employee.operationRole === FINISHED_GOODS_LEGACY_STAGE ? FINISHED_GOODS_LEGACY_ACTION : employee.operationRole,
...
const normalizeProductionAssignmentsData = (assignments = []) => (Array.isArray(assignments) ? assignments : []).map((assignment) => {
  ...
  const lowerTeam = ['Đóng gói', FINISHED_GOODS_LEGACY_STAGE].includes(originalStage) || ['DG', 'KTP'].includes(assignment.teamCode || assignment.productionTeam)
  ...
  const processStages = Array.isArray(assignment.processStages) && assignment.processStages.length
    ? assignment.processStages
    : lowerTeam
      ? ['Đóng gói', FINISHED_GOODS_LEGACY_STAGE]
...
const initialData = {
  ...
  orders: [],
  logs: [
    { id: uid('log'), time: '2026-06-13 08:00', entry: 'Khởi tạo quy trình sản xuất V3: Kho NVL, công thức gốc, QC1, QC-TP, đóng gói và thành phẩm.' },
  ],
  productionLogs: [
    { id: uid('log'), time: '2026-06-13 08:00', entry: 'Khởi tạo quy trình sản xuất V3: Kho NVL, công thức gốc, QC1, QC-TP, đóng gói và thành phẩm.' },
  ],
...
const defaultUsers = [
  ...
  { username: 'kho.tp', password: DEFAULT_PASSWORD, role: 'Kho TP', fullName: 'Thành phẩm' },
...
function statusClass(status = '') {
  if (['PASS', 'OK', 'Completed', 'Hoàn thành', 'Đóng gói', FINISHED_GOODS_LEGACY_STAGE].some((word) => status.includes(word))) return 'pass'
...
function updateMixingMachineStatus(machines = [], machineCode = '', status = 'READY') {
  const normalizedCode = normalizeMixingMachineCode(machineCode)
  if (!normalizedCode) return normalizeMixingMachines(machines)
  return normalizeMixingMachines(machines).map((machine) => (
    machine.machineCode === normalizedCode ? { ...machine, status: normalizeMixingMachineStatus(status) } : machine
  ))
}
...
function buildFinishedProductQrItem(order = {}, item = {}, index = 1, totalBoxes = 0, existing = {}) {
  ...
  const labelId = existing.labelId || `${lot}-${specCode}-${String(sequence).padStart(2, '0')}/${String(totalBoxes).padStart(2, '0')}`
  const qrCode = existing.qrCode || labelId
  ...
  const packageSpec = item.spec || item.label || ''
  const weightKg = num(item.convertedWeightKg ?? item.sizeKg)
  const payload = {
    type: 'FINISHED_GOODS_PACKAGE',
    lotCode: lot,
    ...
    boxIndex: sequence,
    totalBoxes,
    packedWeight: weightKg,
    completedAt: existing.completedAt || existing.packingDate || '',
    labelId,
  ...
  return {
    ...
    qrPayload: { ...(existing.qrPayload || existing.tracePayload || {}), ...payload },
    tracePayload: { ...(existing.qrPayload || existing.tracePayload || {}), ...payload },
    lot,
    orderId: order.id || '',
    orderCode: order.orderCode || order.id || '',
    ...
    reprintLogs: existing.reprintLogs || [],
  }
}
...
function buildFinishedGoodsDemoOrder(config) {
  ...
  const packingLog = {
    ...
    status: 'completed',
    notes: config.notes || 'Dữ liệu demo thành phẩm',
  }
  const order = {
    id: config.id,
    orderCode: config.id,
    ...
    status: FINISHED_GOODS_WAITING_STATUS,
...
function createFinishedGoodsDemoPayload(current = {}) {
  ...
  const productionLog = { id: 'PROD-DEMO-KTP-001', time: nowText(), entry: 'Đã tạo dữ liệu demo thành phẩm.' }
...
const formatMaterialLotLabelDate = (value = '') => {
  const match = String(value || '').slice(0, 10).match(/^(\d{4})-(\d{2})-(\d{2})$/)
  return match ? `${match[3]}/${match[2]}/${match[1]}` : String(value || '-')
}

const formatMaterialLotLabelQuantity = (value, unit = 'kg') => `${new Intl.NumberFormat('vi-VN', { maximumFractionDigits: 3 }).format(num(value))} ${unit || 'kg'}`

function MaterialLotQrLabel({ lot }) {
  if (!lot) return null
  const materialLotCode = normalizeMaterialLotCode(lot.materialLotCode || lot.internalLotCode || lot.internalLot || lot.lotCode)
  const rows = [
    ['Mã vật tư', lot.materialCode || '-'],
    ['Mã lô NL', materialLotCode],
    ['Ngày nhập', formatMaterialLotLabelDate(lot.receiptDate || lot.bravoReceiptDate)],
    ['Số phiếu', lot.receiptNumber || lot.receiptNo || lot.bravoReceiptNo || '-'],
    ['ĐVT', lot.unit || '-'],
    ['Số lượng nhập', formatMaterialLotLabelQuantity(lot.receivedQuantity ?? lot.receivedQty ?? lot.importedQty, lot.unit)],
  ]
  return (
    <section className="material-lot-label">
      <div className="material-lot-label__info">
        {rows.map(([label, value]) => <div className={`material-lot-label__row ${label === 'Mã lô NL' ? 'is-lot-code' : ''}`} key={label}><span>{label}:</span><strong className="material-lot-label__value">{value}</strong></div>)}
      </div>
      <div className="material-lot-label__qr"><QRCodeSVG value={materialLotCode} size={132} includeMargin={false} /><strong>{materialLotCode}</strong></div>
    </section>
  )
}
...
function RawMaterialsPage({ data, setData }) {
  ...
  const previewLotCodeAt = (targetIndex) => {
    const simulatedLots = [...materialLots]
    let code = ''
    importPreviewRows.slice(0, targetIndex + 1).forEach((row) => {
      code = buildInternalLotCode(row.materialCode, row.receiptDate, simulatedLots)
      simulatedLots.push({ internalLotCode: code })
    })
    return code
  }
  const downloadMaterialCatalogTemplate = () => {
    const rows = [
      ['PHIẾU NHẬP KHO'],
      ['stt', 'Mã số', 'ĐVT', 'Số lượng thực nhập', 'Ngày nhập', 'Số phiếu', 'Nhà cung cấp'],
      [1, 'SPB063', 'Kg', 1750, '01/07/2026', 'NM-07/26-001', 'CTY TNHH PRIZE STONE'],
      [2, 'SD11', 'Kg', 2450, '01/07/2026', 'NM-07/26-001', 'CTY TNHH PRIZE STONE'],
    ]
  ...
  const normalizedCell = (value) => normalizeText(cellText(value)).replace(/\u0111/g, 'd').replace(/\u0110/g, 'd')
  const isEmptyImportRow = (row = []) => !row.slice(0, 7).some((cell) => cellText(cell))
  ...
  const isValidReceiptDate = (value = '') => /^\d{4}-\d{2}-\d{2}$/.test(String(value || '')) && !Number.isNaN(Date.parse(`${value}T00:00:00`))
  const validateBravoImportHeader = (header = []) => {
    const expected = ['stt', 'ma so', 'dvt', 'so luong thuc nhap', 'ngay nhap', 'so phieu', 'nha cung cap']
    return expected.every((text, index) => normalizedCell(header[index]) === text)
  }
  const parseBravoReceiptRows = (rows = []) => {
    const titleRow = normalizedCell(rows[0]?.[0])
    const headerIndex = titleRow === 'phieu nhap kho' ? 1 : 0
    const header = rows[headerIndex] || []
    if (!validateBravoImportHeader(header)) throw new Error('Header file import không đúng mẫu.')
    const parsedRows = []
    for (let rowIndex = headerIndex + 1; rowIndex < rows.length; rowIndex += 1) {
      const row = rows[rowIndex] || []
      if (isEmptyImportRow(row)) break
      const parsedRow = {
        stt: cellText(row[0]),
        materialCode: cellText(row[1]),
        unit: cellText(row[2]),
        receivedQty: parseImportQty(row[3]),
        receiptDate: excelDateText(row[4]),
        receiptNo: cellText(row[5]),
        supplierName: cellText(row[6]),
      }
      if (
        !parsedRow.materialCode
        || !parsedRow.unit
        || parsedRow.receivedQty <= 0
        || !isValidReceiptDate(parsedRow.receiptDate)
        || !parsedRow.receiptNo
        || !parsedRow.supplierName
      ) {
        throw new Error(`Dòng số ${rowIndex + 1} không hợp lệ.`)
      }
  ...
  const importBravoRows = (parsedRows = [], sourceLabel = 'Import phiếu nhập Bravo') => {
    ...
    setData((current) => {
      ...
      const hasDuplicate = parsedRows.some((row) => {
        const key = [row.receiptNo, normalizeCode(row.materialCode), row.stt].join('|')
        if (duplicateKeys.has(key) || importKeys.has(key)) return true
        importKeys.add(key)
        return false
      })
      if (hasDuplicate) {
        setNotice('Dữ liệu đã tồn tại.')
        return current
      }
      const nextLots = []
      parsedRows.forEach((row) => {
        ...
        const internalLotCode = buildInternalLotCode(row.materialCode, row.receiptDate, [...existingLots, ...nextLots])
      ...
      setNotice(`Đã import ${nextLots.length} dòng vật tư.`)
      return addLogToData({
        ...current,
        materialLots: [...nextLots, ...existingLots],
      }, `${sourceLabel}: thêm ${nextLots.length} dòng.`)
    })
  ...
  const importMaterialCatalogExcel = (file) => {
    ...
    if (!/\.(xlsx|xls)$/i.test(file.name || '')) {
      setNotice('Chỉ hỗ trợ file Excel chuẩn .xlsx hoặc .xls.')
    ...
    reader.onload = (event) => {
      try {
        ...
        setImportPreviewFileName(file.name || 'Excel Bravo')
        setNotice(`Đã đọc ${parsedRows.length} dòng từ Excel. Vui lòng kiểm tra preview trước khi xác nhận import.`)
      } catch (error) {
        console.error(error)
        setImportPreviewRows([])
        setImportPreviewFileName('')
        setNotice(error?.message || 'Không thể đọc file Bravo. Vui lòng kiểm tra file Excel chuẩn.')
      } finally {
        if (importMaterialCatalogRef.current) importMaterialCatalogRef.current.value = ''
      }
    }
    ...
    <div className="page-content">
      <section className="panel">
        <div className="panel-header-row">
          <div><h2>Nhập lô nguyên liệu</h2><p className="panel-text">Import phiếu nhập nguyên liệu Bravo để sinh Mã lô NL và tem QR cho từng lô.</p></div>
      ...
      {importPreviewRows.length > 0 && (
        <section className="panel">
          <div className="section-heading-row">
            <div><span className="section-kicker">Preview Excel</span><h2>{importPreviewFileName}</h2></div>
            <div className="action-row">
              <button className="primary-button" type="button" onClick={() => importBravoRows(importPreviewRows, 'Import phiếu nhập Bravo từ Excel')}>Xác nhận Import</button>
          ...
          <div className="lot-import-table-wrapper"><SimpleTable tableClassName="lot-import-table" headers={['STT', 'Mã vật tư', 'ĐVT', 'Số lượng', 'Mã lô NL dự kiến', 'Ngày', 'Số phiếu', 'Nhà cung cấp', 'Số dư truy xuất', 'Trạng thái']} rows={importPreviewRows.map((item, previewIndex) => (
            <tr key={`${item.receiptNo}-${item.materialCode}-${item.stt}`}>
              <td>{item.stt}</td>
              <td>{item.materialCode}</td>
              <td>{item.unit}</td>
              <td>{kg(item.receivedQty)}</td>
              <td><code>{previewLotCodeAt(previewIndex)}</code></td>
      ...
      <section className="panel">
        <div className="section-heading-row"><h3>Danh sách lô đã cập nhật</h3><button className="primary-button" type="button" disabled={!selectedLotIds.length} onClick={() => printMaterialLotLabels(materialLots.filter((lot) => selectedLotIdSet.has(lot.id)))}>In tem đã chọn ({selectedLotIds.length} mã)</button></div>
        <div className="lot-import-table-wrapper"><SimpleTable tableClassName="lot-import-table material-lot-list-table" headers={[
          ...
          <tr key={item.id}>
            ...
            <td><div className="action-row"><button className="secondary-button" type="button" onClick={() => setLotQrModal(item)}>Xem tem</button></div></td>
          </tr>
        ))} empty="Chưa có lô nhập Bravo." /></div>
      </section>
      {lotQrModal && (
        <div className="modal-backdrop material-lot-qr-modal" role="presentation">
          <div className="mixing-modal material-lot-qr-dialog" role="dialog" aria-modal="true">
            ...
            <div className="modal-actions"><label className="material-lot-copy-control">Số bản sao<input type="number" min="1" max="99" step="1" value={copyCountFor(lotQrModal)} onChange={(event) => setCopyCount(lotQrModal.id, event.target.value)} /></label><button type="button" className="secondary-button" onClick={() => printMaterialLotLabels([lotQrModal])}>In tem</button><button type="button" className="primary-button" onClick={() => setLotQrModal(null)}>Đóng</button></div>
          </div>
        </div>
      )}
    </div>
    ...
    <div className="page-content">
      <section className="panel">
        ...
        <div className="lot-in-use-table-wrapper">
          <SimpleTable tableClassName="lot-in-use-table" headers={['Mã vật tư', 'Tên vật tư', 'Mã lô NL ưu tiên', 'Ngày nhập', 'Số dư truy xuất', 'Lô tiếp theo', 'Trạng thái']} rows={rows.map(({ materialCode, current, next }) => (
            <tr key={materialCode}>
              <td>{materialCode}</td>
  ...
  return (
    <div className="page-content">
      <section className="panel">
        ...
        <div className="material-lot-adjustment-form">
          <div className="material-lot-adjustment-main-row">
            ...
            <label className="adjustment-lot-field">Mã lô NL<select value={form.internalLotCode} onChange={(event) => updateForm('internalLotCode', event.target.value)}><option value="">Chọn lô</option>{selectedLots.map((lot) => <option key={lot.internalLotCode} value={lot.internalLotCode}>{lot.internalLotCode}</option>)}</select></label>
      ...
      <section className="panel">
        ...
        <SimpleTable tableClassName="material-lot-adjustment-history-table" headers={['Thời gian', 'Mã lô NL', 'Mã vật tư', 'Loại', 'Trước', 'Điều chỉnh', 'Sau', 'Lý do', 'Người điều chỉnh']} rows={normalizeMaterialTraceAdjustments(data.materialTraceAdjustments || []).map((item) => (
  ...
  return (
    <div className="page-content">
      ...
      <section className="panel">
        ...
        <SimpleTable tableClassName="formula-detail-table" headers={['STT', 'Mã vật tư', 'Nhóm', 'Phân nhóm', 'Tỷ lệ %', 'Dung sai', 'Ghi chú']} rows={(selected.items || []).map((item, index) => (
          <tr key={item.id}>
            <td>{index + 1}</td>
            <td>{item.materialCode}</td>
...
function OrdersPage({ data, setData, permissions = [] }) {
  ...
  const canAdjustProductionPriority = (order = {}) => {
    ...
    const lockedStatus = new RegExp(`Đang cân|Đã cân|Đang phối trộn|Chờ QC-TP|Đang QC-TP|Chờ QC thành phẩm|Đang QC thành phẩm|Đang đóng gói|${FINISHED_GOODS_WAITING_STATUS}|Hoàn thành`, 'i').test(statusText)
    const relatedContainers = (data.weighedContainers || []).some((container) => container.orderId === order.id || container.orderCode === order.orderCode || container.lot === getOrderLotCode(order))
...
function ChemicalWeighingGroupBoard({ board = {}, activeOrder, updateWeight, rawMaterialLots = [], materialCatalog = [], setWarning, scaleWeighedBy }) {
  ...
  return (
    <section className="chemical-weighing-group">
      ...
      <SimpleTable
        ...
        headers={['STT', 'Mã vật tư', 'KL cần cân', 'Đã cân', 'Dung sai', 'Quét QR Mã lô NL', 'Số dư truy xuất', 'Thực cân', 'Trạng thái', 'Thao tác']}
...
function WeighingPage({ data = {}, setData, group, user }) {
  ...
  const renderChemicalBoard = (board) => board ? (
    <ChemicalWeighingGroupBoard key={board.key} board={board} activeOrder={activeOrder} updateWeight={updateWeight} rawMaterialLots={normalizeMaterialLots(data.materialLots || [])} materialCatalog={materialCatalog} setWarning={setWarning} scaleWeighedBy={scaleWeighedBy} />
  ) : null
        ...
        <main className="weighing-active-board scale-main-panel">
            ...
            <div className="action-row">
              {group !== CHEMICAL && (
                isScaleConnected
                  ? (
                    <>
                      <button className="secondary-button weighing-finish-button" type="button" disabled>Đã kết nối cân</button>
                      <button className="secondary-button weighing-finish-button" type="button" onClick={disconnectScale}>Ngắt kết nối</button>
                    </>
                  )
          ...
          {activeOrder && (
            <>
              ...
              {group !== CHEMICAL && (
                <SimpleTable tableClassName="solid-weighing-table material-weighing-table" headers={['STT', 'Mã vật tư', 'KL cần cân', 'Đã cân', 'Dung sai', 'Quét QR Mã lô NL', 'Số dư truy xuất', 'Thực cân', 'Trạng thái', 'Thao tác']} rows={activeItems.map((item, index) => (
                  <WeighingRow key={`${activeOrder.id}-${item.id}`} order={activeOrder} item={item} index={index} active={isSameWeighingItem(item, activeItem)} updateWeight={updateWeight} rawMaterialLots={normalizeMaterialLots(data.materialLots || [])} materialCatalog={materialCatalog} scaleType={scaleKey} setWarning={setWarning} scaleWeightKg={scaleWeightKg} scaleStableWeightKg={scaleStableWeightKg} scaleStable={scaleStableWeightKg != null} scaleRawData={scaleRawText} scaleRawValue={scaleRawValue} scaleWeighedBy={scaleWeighedBy} />
...
function getAccumulatedWeighingState({ active = true, qrMatched = false, weightPassed = false, currentWeight = 0, accumulatedWeight = 0, requiredWeight = 0, tolerance = 0 } = {}) {
  ...
  const canUseCurrentWeight = active && qrMatched && !weightPassed
  const isBelowTolerance = hasTolerance && totalWeight < minWeight
  const isWithinTolerance = hasTolerance && totalWeight >= minWeight && totalWeight <= maxWeight
  const isAboveTolerance = hasTolerance && totalWeight > maxWeight
  ...
  return {
    ...
    isWithinTolerance,
    isAboveTolerance,
    canContinueWeighing: canUseCurrentWeight && isBelowTolerance,
    canConfirmAccumulatedWeight: canUseCurrentWeight && isWithinTolerance,
  }
}
...
function WeighingRow({ order, item, index, active, updateWeight, rawMaterialLots = [], scaleType, setWarning, scaleWeightKg = null, scaleStableWeightKg = null, scaleStable = false, scaleRawData = '', scaleRawValue = null, scaleWeighedBy = '', showActionColumn = true, onActionStateChange = null }) {
  ...
  const selectedLot = rawMaterialLots.find((lot) => (lot.internalLotCode || lot.lotCode) === item.rawMaterialLotCode && lot.materialCode === item.materialCode)
  const remainingBefore = item.rawMaterialRemainingBefore ?? selectedLot?.traceBalanceQty ?? selectedLot?.remainingQty
  const materialStockQty = rawMaterialLots
    .filter((lot) => normalizeCode(lot.materialCode) === normalizeCode(item.materialCode))
    .reduce((sum, lot) => sum + num(lot.traceBalanceQty ?? lot.remainingQty), 0)
  const stockDisplay = active && !weightPassed
    ? (selectedLot?.traceBalanceQty ?? selectedLot?.remainingQty ?? remainingBefore ?? (materialStockQty ? materialStockQty : ''))
    : (remainingBefore ?? (materialStockQty ? materialStockQty : ''))
  const confirmQr = (nextInput = qrInput) => {
    if (!active) return
    const scanned = String(nextInput || '').trim()
    if (!scanned) {
      setWarning?.('Vui lòng quét hoặc nhập Mã lô NL.')
      return
    }
    ...
    updateWeight(order, item, {
      ...
      rawMaterialLotCode: result.lot.internalLotCode,
      materialLotCode: result.lot.internalLotCode,
      rawMaterialRemainingBefore: num(result.lot.traceBalanceQty),
      materialQrOnly: false,
      qrStatus: 'PASS',
      qrMatchStatus: 'OK',
      qrMatched: true,
      ...
      note: item.note,
    }, `QR PASS ${order.id} - ${item.materialCode}, Mã lô NL ${result.lot.internalLotCode}.`)
  }
  ...
  const getValidatedWeighingContext = (scaleWeight = null) => {
    ...
    const lot = rawMaterialLots.find((row) => (row.internalLotCode || row.lotCode) === item.rawMaterialLotCode && row.materialCode === item.materialCode)
    ...
    if (num(lot.traceBalanceQty ?? lot.remainingQty) <= 0) {
      setWarning?.('Lô nguyên liệu đã hết số dư truy xuất.')
      return null
    }
    if (num(lot.traceBalanceQty ?? lot.remainingQty) < actualWeight) {
      setWarning?.('Lô nguyên liệu không đủ số dư truy xuất.')
      return null
    }
    const nextRemaining = num(lot.traceBalanceQty ?? lot.remainingQty) - actualWeight
    return {
      actualWeight,
      totalWeight,
      supplementalText,
      ...
      stockIssue: {
        productionOrderId: order.id,
        materialCode: item.materialCode,
        lotCode: lot.internalLotCode || lot.lotCode,
  ...
  const continueWeighing = (scaleWeight = null) => {
    ...
    updateWeight(order, item, {
      ...
      weighStatus: 'Chờ cân',
      ...(context.lot ? {
        rawMaterialRemainingBefore: num(context.lot.traceBalanceQty ?? context.lot.remainingQty),
        rawMaterialRemainingAfter: context.nextRemaining,
        ...(context.nextRemaining <= 0 ? { qrScanned: '', rawMaterialQr: '', rawMaterialLotCode: '', materialLotCode: '', qrStatus: '', qrMatchStatus: '', qrMatched: false } : {}),
      } : {}),
  ...
  const confirmWeight = (scaleWeight = null) => {
    ...
    updateWeight(order, item, {
      ...
      ...context.scaleAudit,
      note: item.note,
    }, `${context.supplementalText} PASS ${order.id} - ${item.materialCode}${context.lot ? `, Mã lô NL ${context.lot.internalLotCode || context.lot.lotCode}` : ''}.`, context.stockIssue)
  }
  ...
  return (
    <tr className={`${active ? 'weighing-active-row' : ''} ${completed ? 'weighing-completed-row' : ''} ${totalAboveTolerance || qrFailed || (qrPassed && !hasTolerance) ? 'weighing-weight-fail-row' : ''} ${qrPassed ? 'weighing-qr-pass-row' : ''} ${qrFailed ? 'weighing-qr-fail-row' : ''}`}>
      <td>{index + 1}</td>
      <td>{item.materialCode}</td>
...
function getMixingDispatchState(order) {
  ...
  if (!chemicalDone || !solidDone) return { label: 'Chờ cân', className: 'waiting', canStart: false }
...
function MixingPage({ data, setData, user }) {
  ...
  const getMixingQrScan = (order, group) => {
    ...
    const state = qrScanStates[order.id]?.[stateKey] || null
    const hasScanEvidence = Boolean(order.mixingQrConfirmation?.[`${field}ScannedAt`] || state?.status === 'pass')
    const validation = qrCode && hasScanEvidence ? validateSingleMixingQr(order, qrCode, group) : null
    return {
      pass: Boolean(hasScanEvidence && validation?.pass),
      qrCode,
  ...
  const getMachineQrScan = (order) => {
    ...
    const hasScanEvidence = Boolean(order.mixingQrConfirmation?.machineQrScannedAt || state?.status === 'pass')
    const validation = qrCode && hasScanEvidence ? validateMixingMachineQr(order, qrCode) : null
    return {
      pass: Boolean(hasScanEvidence && validation?.pass),
      qrCode,
      machine: validation?.machine || null,
      state,
      message: state?.message || validation?.message || '',
    }
  }
  ...
  const validateSingleMixingQr = (order, qrText, group) => {
    ...
    const usedByOtherOrder = orders.some((item) => item.id !== order.id && [
      item.mixingQrConfirmation?.chemicalQrScannedAt || item.mixingStartAt || item.mixing?.startedAt ? item.mixingQrConfirmation?.chemicalQr : '',
      item.mixingQrConfirmation?.solidQrScannedAt || item.mixingStartAt || item.mixing?.startedAt ? item.mixingQrConfirmation?.solidQr : '',
    ].filter(Boolean).map((value) => String(value).trim().toUpperCase()).includes(normalizedQr))
    const usedByThisOrder = Boolean(order.mixingStartAt || order.mixing?.startedAt || ['Active', 'completed'].includes(order.mixingStatus) || ['Active', 'Completed'].includes(order.mixing?.status))
    const wrongType = !normalizedQr.startsWith(expectedPrefix)
    const pass = Boolean(
      container
      && sameOrder
      && sameLot
    ...
    const finalPass = pass && !wrongType && !usedByThisOrder
    const label = group === CHEMICAL ? 'QR Hóa' : 'QR Rắn'
    const message = finalPass
      ? ''
      : !container ? `${label} không tồn tại.`
        : wrongType ? `${label} sai loại.`
          : usedByThisOrder ? `${label} đã được dùng.`
            : usedByOtherOrder ? `${label} đã được dùng cho lệnh khác.`
  ...
  const validateMixingMachineQr = (order, qrText) => {
    const rawQr = String(qrText || '').trim().toUpperCase()
    const machineCode = normalizeMachineQrCode(rawQr)
    const assignedMachineCode = normalizeMixingMachineCode(getAssignedMachineCode(order))
  ...
  const getMixingStartValidation = (order) => {
    ...
    const messages = [
      ...
      !solidOk ? (solidScan.message || 'Chờ quét MIX Rắn.') : '',
    ].filter(Boolean)
    return { ok: chemicalOk && solidOk, chemicalScan, solidScan, requiresChemicalQr: requiresChemical, requiresSolidQr: requiresSolid, messages }
  }
  const scanMixingQr = (order, group, value) => {
    ...
    if (!result.pass) {
      const failMessage = result.message || (group === CHEMICAL ? 'Sai QR Hóa' : 'Sai QR Rắn')
      setQrScanStates((current) => ({
        ...current,
        [order.id]: {
          ...(current[order.id] || {}),
          [stateKey]: { status: 'fail', scannedAt, message: failMessage },
        },
      }))
      setWarning(failMessage)
      return
    }
    setQrScanStates((current) => ({
      ...current,
    ...
    setData((current) => {
      const nextOrders = current.orders.map((item) => {
        ...
        const otherField = field === 'chemicalQr' ? 'solidQr' : 'chemicalQr'
        const otherScanned = Boolean(currentConfirmation[`${otherField}ScannedAt`])
        const nextConfirmation = {
          ...currentConfirmation,
          [field]: result.container.qrCode || qrText,
          [`${field}ScannedAt`]: scannedAt,
          status: field === 'chemicalQr'
            ? (otherScanned ? 'Đã xác nhận' : 'Chờ QR Rắn')
            : (otherScanned ? 'Đã xác nhận' : 'Chờ QR Hóa'),
          confirmedAt: field === 'chemicalQr'
            ? (otherScanned ? scannedAt : currentConfirmation.confirmedAt || '')
            : (otherScanned ? scannedAt : currentConfirmation.confirmedAt || ''),
          note: 'QR hỗn hợp đã được xác thực khi quét.',
        }
  ...
  const scanMixingMachineQr = (order, value) => {
    const qrText = String(value || '').trim()
    if (!qrText) return
    const result = validateMixingMachineQr(order, qrText)
    const scannedAt = nowText()
    resetQrInput(order.id, 'machineQr')
    if (!result.pass) {
      setQrScanStates((current) => ({
        ...current,
        [order.id]: {
  ...
  const handleMixingMachineQrInputKeyDown = (event, order) => {
    if (event.key !== 'Enter') return
    event.preventDefault()
    scanMixingMachineQr(order, event.currentTarget.value)
  }
  const handleMixingMachineQrInputBlur = (event, order) => {
    if (event.currentTarget.value.trim()) scanMixingMachineQr(order, event.currentTarget.value)
  }
  const renderMixQrCell = ({ order, group, flow, scan, requiresQr, fail, isMixingActive }) => {
    const isChemical = group === CHEMICAL
    const field = qrFieldByGroup(group)
    const okText = isChemical ? '✓ OK Hóa' : '✓ OK Rắn'
    const waitingText = isChemical ? 'Chờ quét MIX Hóa' : 'Chờ quét MIX Rắn'
    const failText = isChemical ? '✕ Sai QR Hóa' : '✕ Sai QR Rắn'
    const handleKeyDown = (event) => handleMixingQrInputKeyDown(event, order, group)
    const handleBlur = (event) => handleMixingQrInputBlur(event, order, group)
    if (isMixingActive) return <span className="weighing-check">{okText}</span>
    if (flow.status === 'not_required') return <span className="dispatch-badge locked">Không yêu cầu</span>
    if (flow.status === 'pending') return <span className="dispatch-badge waiting">Chưa cân</span>
    if (flow.status === 'weighing') return <span className="dispatch-badge mixing">Đang cân</span>
    if (requiresQr && scan.pass) return <span className="weighing-check">{okText}</span>
    if (requiresQr) {
      return (
        <>
          {fail && <><span className="dispatch-badge fail">{failText}</span><small>{scan.state?.message || scan.message || 'Không đúng lệnh này'}</small></>}
          <input className="mixing-qr-input qr-input" value={qrForms[order.id]?.[field] || ''} onChange={(event) => updateQrForm(order.id, field, event.target.value)} onKeyDown={handleKeyDown} onBlur={handleBlur} placeholder={waitingText} />
        </>
      )
    }
    return <span className="dispatch-badge waiting">Chưa cân</span>
  }
  const createDemoQrData = () => {
    setData((current) => applyDemoQrData(current))
  ...
  const startMixing = (order) => {
    ...
    if (!machineCode) {
      setWarning('Lệnh chưa chỉ định máy trộn')
      return
    }
    ...
    setData((current) => addLogToData({
      ...
      mixingMachines: updateMixingMachineStatus(current.mixingMachines, machineCode, 'RUNNING'),
  ...
  const completeMixing = (order, supplement) => setData((current) => addLogToData({
    ...
    mixingMachines: updateMixingMachineStatus(current.mixingMachines, order.mixingMachine || order.mixing?.machineCode || getOrderAssignedMachineCode(order), 'READY'),
    orders: current.orders.map((item) => item.id === order.id ? {
      ...item,
      stage: 'finished-qc',
      status: 'Chờ QC thành phẩm',
            ...
            <table className="ready-mixing-table mixing-order-compact-table">
              ...
              <tbody>
                {readyOrders.map((order) => {
                  ...
                  const qrReady = startValidation.ok
                    ...
                    <tr key={order.id} className={!isMixingActive && qrFailed ? 'mixing-qr-fail-row' : ''}>
                      ...
                      <td>
                        <div className="mixing-scan-cell">
                          {renderMixQrCell({ order, group: CHEMICAL, flow: chemicalFlow, scan: chemicalScan, requiresQr: requiresChemicalQr, fail: chemicalFail, isMixingActive })}
                        </div>
                      </td>
                      <td>
                        <div className="mixing-scan-cell">
                          {renderMixQrCell({ order, group: SOLID, flow: solidFlow, scan: solidScan, requiresQr: requiresSolidQr, fail: solidFail, isMixingActive })}
                        </div>
                      </td>
                      <td><span className={`dispatch-badge ${isMixingActive ? 'mixing' : state.canStart && qrReady ? 'ready' : qrFailed ? 'fail' : state.className}`}>{isMixingActive ? 'Đang phối trộn' : state.canStart ? (qrFailed ? (startValidation.messages[0] || 'Sai QR') : qrReady ? 'Sẵn sàng phối trộn' : 'Chờ quét QR') : state.label}</span></td>
              ...
              {readyOrders.length === 0 && <tr><td className="empty-row" colSpan={7}>Chưa có lệnh phối trộn.</td></tr>}
              </tbody>
            </table>
          </div>
        </section>
        <section className="panel mixing-active-section">
          ...
          <div className="table-wrapper mixing-active-table-wrapper">
            <table className="mixing-active-table">
              ...
              {activeMixingOrders.length === 0 && <tr><td className="empty-row" colSpan={7}>Chưa có lệnh đang phối trộn.</td></tr>}
              </tbody>
            </table>
          </div>
        </section>
...
function PackagingPage({ data, setData, user }) {
  ...
  const normalizeFinishedGoodsQrLabel = (label = {}) => {
    const boxIndex = Math.max(1, num(label.boxIndex ?? label.labelNo ?? label.sequence ?? 1))
    const totalBoxes = Math.max(1, num(label.totalBoxes ?? label.boxQty ?? label.packedQty ?? label.boxes ?? 1))
    const packedWeight = num(label.packedWeight ?? label.weightKg ?? label.actualWeight ?? label.convertedWeight)
    const packageSpec = String(label.packageSpec || label.packagingSpec || label.spec || label.label || '').trim()
    const payload = {
      type: 'FINISHED_GOODS_PACKAGE',
      lotCode: String(label.lotCode || label.lot || '').trim(),
      productCode: String(label.productCode || '').trim(),
      productName: String(label.productName || '').trim(),
      packageSpec,
      boxIndex,
      totalBoxes,
      packedWeight,
      completedAt: String(label.completedAt || label.packingDate || '').trim(),
      labelId: label.labelId || label.qrCode || '',
    }
    return {
      ...payload,
      labelNo: boxIndex,
      ...
      printCount: num(label.printCount),
      printedAt: label.printedAt || '',
      printedBy: label.printedBy || '',
      reprintLogs: label.reprintLogs || [],
  ...
  const buildFinishedGoodsQrPayload = (log = {}, order = {}) => {
    ...
    const details = (log.packingDetails || order?.packaging?.details || []).filter(isStandardPackagingItem)
    const packageSpec = packingSpecSummary(details)
  ...
  const labelsFromFinishedGoodsQrItems = (items = []) => items.map((qr) => normalizeFinishedGoodsQrLabel({
    lotCode: qr.lot,
    productCode: qr.productCode,
    productName: qr.productName,
    packageSpec: qr.spec,
    boxIndex: qr.sequence,
    totalBoxes: qr.totalBoxes || 1,
    packedWeight: qr.weightKg,
    completedAt: qr.packingDate || form.completedAt || nowText(),
    labelId: qr.labelId || qr.qrCode,
    qrPayload: qr.qrPayload || qr.tracePayload,
    productionDate: qr.productionDate,
    expiryDate: qr.expiryDate,
    printCount: qr.printCount,
    printedAt: qr.printedAt,
    printedBy: qr.printedBy,
    reprintLogs: qr.reprintLogs,
  }))
  const generateFinishedGoodsQrLabels = ({ labels = [], qrItems = [], log = null, order = null } = {}) => {
    const saved = labels.length ? labels : qrItems.length ? qrItems : log?.qrLabels
  ...
  const openFinishedGoodsQrModal = (labels = [], spec = '', historyLogId = '') => {
    ...
    setPrintModal({ detailId: '', spec: spec || 'Tem thành phẩm', boxes: qrItems.length, qrItems, reprint: false, history: true, historyLogId, singleLabel: qrItems.length === 1 })
  }
  const labelsFromPackingLog = (log = {}, order = {}) => {
    const payloadBase = buildFinishedGoodsQrPayload(log, order)
    const details = (log.packingDetails || order?.packaging?.details || []).filter((item) => isStandardPackagingItem(item) && num(item.boxes) > 0)
    if (!details.length) return [normalizeFinishedGoodsQrLabel(payloadBase)]
    return details.flatMap((detail) => {
      const boxQty = Math.max(1, num(detail.boxes))
      const packedWeight = num(detail.actualWeight || detail.convertedWeight || num(detail.convertedWeightKg ?? detail.sizeKg) * boxQty)
      return Array.from({ length: boxQty }, (_, index) => normalizeFinishedGoodsQrLabel({
        ...payloadBase,
        packageSpec: detail.spec || detail.label || payloadBase.packageSpec,
        boxQty,
        packedWeight: Number((packedWeight / boxQty).toFixed(3)),
        labelNo: index + 1,
      }))
    })
  }
  const renderFinishedGoodsQrDataUrls = async (labels = []) => {
    ...
    root.render(
      <div>
        {labels.map((label, index) => (
          <QRCodeCanvas key={label.labelId || `${label.lotCode}-${label.packageSpec}-${label.labelNo || index}`} value={JSON.stringify(label.qrPayload || label)} size={360} includeMargin />
        ))}
      </div>,
    )
    await new Promise((resolve) => setTimeout(resolve, 80))
    const dataUrls = Array.from(host.querySelectorAll('canvas')).map((canvas) => canvas.toDataURL('image/png')).filter(Boolean)
    root.unmount()
    document.body.removeChild(host)
    return dataUrls
  }
  const printFinishedGoodsQrLabels = async (dataToPrint = []) => {
    if (typeof document === 'undefined') return
    const labels = (Array.isArray(dataToPrint) ? dataToPrint : generateFinishedGoodsQrLabels(dataToPrint))
      .map(normalizeFinishedGoodsQrLabel)
      .filter((label) => label.lotCode && (label.productCode || label.productName) && label.packedWeight > 0)
    if (!labels.length) {
      setWarning('Không có dữ liệu QR để in.')
      return
    }
    const qrImages = await renderFinishedGoodsQrDataUrls(labels)
    if (qrImages.length !== labels.length) {
      setWarning('Không có dữ liệu QR để in.')
      return
    }
    ...
    const labelHtml = labels.map((label, index) => {
      ...
      const rowHtml = rows.map(([rowLabel, value]) => `<div class="qr-row"><span>${escapeHtml(rowLabel)}</span><strong>${escapeHtml(value)}</strong></div>`).join('')
      return `<div class="qr-label">
          <div class="qr-info">
            <h1 class="qr-title">${escapeHtml(label.productCode || label.productName || 'Lô thành phẩm')}</h1>
            ${rowHtml}
          </div>
          <div class="qr-box"><img src="${qrImages[index]}" alt="QR lô thành phẩm"></div>
        </div>`
    }).join('')
        ...
        <head>
          ...
          <style>
            ...
            * { box-sizing: border-box; }
            ...
            .qr-title { margin: 0; font-size: 10px; font-weight: 800; line-height: 1.1; overflow-wrap: anywhere; }
            .qr-row { display: grid; grid-template-columns: 14mm 1fr; gap: 1mm; align-items: baseline; }
  ...
  const printSelectedLabels = () => {
    ...
    if (printModal.history) {
      const printedAt = nowText()
      const printedBy = user?.fullName || user?.name || user?.username || user?.role || 'Đóng gói'
      const printedLabels = generateFinishedGoodsQrLabels({ qrItems: items }).map((label) => ({
        ...label,
        printCount: num(label.printCount) + 1,
        printedAt,
        printedBy,
        reprintLogs: [...(label.reprintLogs || []), { printedAt, printedBy, reason: 'In lại tem' }],
      }))
      if (printModal.historyLogId) {
        const printedById = new Map(printedLabels.map((label) => [label.labelId, label]))
        setData((current) => ({
          ...current,
          packingLogs: (current.packingLogs || []).map((log) => log.packingId === printModal.historyLogId ? {
            ...log,
            qrLabels: generateFinishedGoodsQrLabels({ log }).map((label) => printedById.get(label.labelId) || label),
          } : log),
        }))
        setHistoryDetail((current) => current?.log?.packingId === printModal.historyLogId ? {
          ...current,
          log: { ...current.log, qrLabels: generateFinishedGoodsQrLabels({ log: current.log }).map((label) => printedById.get(label.labelId) || label) },
        } : current)
      }
      printFinishedGoodsQrLabels(printedLabels)
      return
    }
  ...
  const buildPackingLog = (order, nextForm, status) => {
    ...
    const completedAt = nextForm.completedAt || nowText()
    const finishedQrItems = buildFinishedQrItems(order, nextForm.details).map((qr) => ({
      ...qr,
      packingDate: qr.packingDate || String(completedAt).slice(0, 10),
      completedAt,
      qrPayload: { ...(qr.qrPayload || qr.tracePayload || {}), completedAt },
      tracePayload: { ...(qr.qrPayload || qr.tracePayload || {}), completedAt },
    }))
    const detailsWithQr = attachFinishedQrItemsToDetails(nextForm.details, finishedQrItems)
    const qrLabels = generateFinishedGoodsQrLabels({ qrItems: finishedQrItems })
  ...
  const completePackaging = () => {
    ...
    }, `Hoàn tất đóng gói lệnh ${activeOrder.id}, chuyển ${FINISHED_GOODS_WAITING_STATUS}.`, operationLogMeta(user, { assignments: currentAssignments, employee: assignmentEmployeeText, stage: 'Đóng gói', order: activeOrder, result: 'Hoàn tất đóng gói' })))
    setWarning('')
  }
            ...
            <>
              <div className="section-heading-row">
                <div>
                  ...
                  <h2>{getOrderLotCode(activeOrder)}</h2>
                </div>
              ...
              {printModal && (
                <div className="modal-backdrop" role="presentation">
                  <div className="production-modal packaging-print-modal" role="dialog" aria-modal="true">
                    ...
                    <div className="packaging-print-body">
                      {printModal.reprint && (
                        <section className="packaging-reprint-options" aria-label="Chọn chế độ in">
                          ...
                          <label className="packaging-reprint-choice">
                            ...
                            <span>In từ:</span>
                            <input type="number" min="1" max={printModal.boxes} value={reprintChoice.from} disabled={reprintChoice.mode !== 'range'} onChange={(event) => setReprintChoice({ ...reprintChoice, mode: 'range', from: event.target.value })} />
                            <span>đến:</span>
                            <input type="number" min="1" max={printModal.boxes} value={reprintChoice.to} disabled={reprintChoice.mode !== 'range'} onChange={(event) => setReprintChoice({ ...reprintChoice, mode: 'range', to: event.target.value })} />
                          </label>
                        </section>
                      )}
                    ...
                    <div className="modal-actions packaging-print-actions">
                      <button type="button" className="primary-button" onClick={printSelectedLabels}>{printModal.singleLabel ? 'In lại tem này' : 'In tem'}</button>
                      <button type="button" className="secondary-button" onClick={() => setPrintModal(null)}>Đóng</button>
                    </div>
                  </div>
                </div>
      ...
      {!activeOrder && printModal && (
        <div className="modal-backdrop" role="presentation">
          <div className="production-modal packaging-print-modal" role="dialog" aria-modal="true">
            ...
            <div className="modal-actions packaging-print-actions"><button type="button" className="primary-button" onClick={printSelectedLabels}>{printModal.singleLabel ? 'In lại tem này' : 'In tem'}</button><button type="button" className="secondary-button" onClick={() => setPrintModal(null)}>Đóng</button></div>
          </div>
        </div>
      )}
      {historyDetail && (() => {
        ...
        return <div className="modal-backdrop" role="presentation"><div className="production-modal packaging-history-detail-modal" role="dialog" aria-modal="true"><div className="modal-header"><div><span className="section-kicker">Chi tiết đóng gói</span><h2>{log.lot || getOrderLotCode(order || {}) || log.orderCode}</h2></div><button type="button" className="icon-button" onClick={() => setHistoryDetail(null)} aria-label="Đóng">×</button></div><div className="qc-order-summary"><div><span>Trạng thái</span><strong>Đã đóng gói</strong></div><div><span>Mã SP</span><strong>{order?.productCode || log.productCode || '-'}</strong></div><div><span>Sản phẩm</span><strong>{order?.productName || log.productName || '-'}</strong></div><div><span>Khách hàng</span><strong>{order?.customerName || order?.customer || '-'}</strong></div><div><span>KL QC-TP</span><strong>{kg(log.qc2FinalWeight)}</strong></div><div><span>KL đã đóng gói</span><strong>{kg(log.totalPackedWeight)}</strong></div><div><span>Dư SX</span><strong>{kg(Math.max(0, num(log.remainingWeight)))}</strong></div><div><span>Người đóng gói</span><strong>{log.packer || '-'}</strong></div><div><span>Bắt đầu / hoàn tất</span><strong>{log.startedAt || '-'} / {log.completedAt || '-'}</strong></div></div><p className="panel-text">Ghi chú: {log.notes || '-'}</p><SimpleTable headers={['Quy cách', 'Số thùng', 'Khối lượng quy đổi', 'Số tem đã in']} rows={details.map((item) => <tr key={item.specId || item.spec}><td>{item.spec || item.label || '-'}</td><td>{num(item.boxes)}</td><td>{kg(item.convertedWeight || item.actualWeight)}</td><td>{(item.finishedQrItems || []).length}</td></tr>)} empty="Chưa có quy cách đã đóng gói." /><SimpleTable headers={['STT', 'Mã tem', 'Quy cách', 'Thứ tự', 'Khối lượng', 'Trạng thái', 'Số lần in', 'In lần cuối', 'Hành động']} rows={labels.map((label, index) => <tr key={label.labelId}><td>{index + 1}</td><td>{label.labelId || '-'}</td><td>{label.packageSpec || '-'}</td><td>{label.boxIndex}/{label.totalBoxes}</td><td>{kg(label.packedWeight)}</td><td>{label.printedAt ? 'Đã in' : 'Chưa in'}</td><td>{num(label.printCount)}</td><td>{label.printedAt || '-'}</td><td><div className="action-row compact-actions"><button type="button" className="secondary-button" onClick={() => openFinishedGoodsQrModal([label], label.packageSpec, log.packingId)}>Xem tem</button><button type="button" className="primary-button" onClick={() => openFinishedGoodsQrModal([label], label.packageSpec, log.packingId)}>In lại</button></div></td></tr>)} empty="Chưa có QR labels." /><div className="modal-actions"><button type="button" className="secondary-button" onClick={() => openFinishedGoodsQrModal(labels, '', log.packingId)}>In tất cả tem</button><button type="button" className="secondary-button" onClick={() => setHistoryDetail(null)}>Đóng</button></div></div></div>
      })()}
    </div>
  )
}
...
function FinishedGoodsPage({ data, setData, user }) {
  ...
  const waitingOrders = data.orders
    .filter((order) => (
      ...
      && (order.orderStatus === FINISHED_GOODS_WAITING_STATUS || order.status === FINISHED_GOODS_WAITING_STATUS || order.stage === 'finished-goods')
    ))
    .slice()
    .sort(sortOldestOrders)
  ...
  const createDemoData = () => {
    ...
    setNotice('Đã tạo dữ liệu demo thành phẩm')
  }
  ...
  return (
    <div className="page-content finished-goods-page">
      <section className="panel">
        <div className="section-heading-row">
          <h2>Danh sách lệnh chờ ghi nhận thành phẩm</h2>
          <button className="primary-button" onClick={createDemoData}>Tạo dữ liệu demo thành phẩm</button>
        </div>
        {notice && <div className="process-alert success-alert">{notice}</div>}
        <div className="finished-warehouse-table-wrapper warehouse-table-wrapper">
          <table className="finished-warehouse-table warehouse-table">
            ...
            <tbody>
              ...
              {waitingOrders.length === 0 && <tr><td className="empty-row" colSpan={10}>Chưa có lệnh chờ ghi nhận thành phẩm.</td></tr>}
            </tbody>
          </table>
        </div>
      </section>
        ...
        <div className="finished-goods-table-wrapper warehouse-table-wrapper">
          <table className={`finished-goods-table warehouse-table ${showWarehouseLocation ? 'show-location' : 'hide-location'}`}>
            ...
            <tbody>
              ...
              {filteredFinishedGoods.length === 0 && <tr><td className="empty-row" colSpan={showWarehouseLocation ? 9 : 8}>Chưa có dữ liệu thành phẩm phù hợp.</td></tr>}
            </tbody>
          </table>
        </div>
      </section>
      ...
      <section className="panel finished-goods-request-panel">
        ...
        {showRequestVoucher && (
          <div className="warehouse-table-wrapper finished-voucher-preview no-print">
      ...
      {form && activeOrder && (
        <div className="modal-backdrop" role="presentation">
          <div className="mixing-modal finished-goods-modal" role="dialog" aria-modal="true">
            <div className="modal-header">
              <div><span className="section-kicker">Ghi nhận thành phẩm</span><h2>{activeOrder.orderCode || activeOrder.id}</h2></div>
            ...
            <div className="production-form-grid">
              ...
              <label>Người ghi nhận<input value={form.receiver} onChange={(event) => updateForm('receiver', event.target.value)} /></label>
              <label>Ngày ghi nhận<input type="date" value={form.importDate} onChange={(event) => updateForm('importDate', event.target.value)} /></label>
            ...
            <div className="modal-actions">
              ...
              <button type="button" className="primary-button" onClick={completeImport}>Hoàn tất ghi nhận</button>
            </div>
          </div>
        </div>
      )}
...
function historyInfoRows(record) {
  ...
  return [
    ...
    ['Khối lượng thành phẩm', kg(imported)],
...
function inferLogStage(log = {}) {
  ...
  if (/nhập kho|thành phẩm/i.test(text)) return 'Thành phẩm'
  return '-'
}
...
function ReportsPage({ data, initialTab = 'production', lockedTab = false }) {
  ...
  const orderCodeText = (order = {}) => getOrderLotCode(order)
  ...
  const orderStageStatus = (order = {}, stage) => {
    ...
    if (stage === FINISHED_GOODS_LEGACY_STAGE) return finishedGoods.some((item) => item.orderId === order.id || item.orderCode === orderCodeText(order)) || order.stage === 'completed' ? 'Hoàn thành' : '-'
    return '-'
  }
  ...
  const productionRows = filteredProductionOrders.map((order) => {
    ...
    return {
      ...
      warehouse: orderStageStatus(order, FINISHED_GOODS_LEGACY_STAGE),
  ...
  const exportProductionExcel = () => {
    ...
    const sheetRows = [
      ...
      [],
      headers,
      ...productionRows.map((row) => [row.orderCode, row.createdAt, row.product, row.formula, row.plannedWeight, row.actualWeight || '', row.machine, row.qc1, row.chemical, row.solid, row.mixing, row.qc2, row.packaging, row.warehouse, row.status, row.latestActor, row.latestTime]),
    ]
    const book = XLSX.utils.book_new()
  ...
  const traceRows = traceRecords.map((record) => {
    ...
    return {
      ...
      warehouseActor: actorTextByStage(record, new RegExp(`Nhập kho|${FINISHED_GOODS_LEGACY_STAGE}`, 'i')),
  ...
  const exportTraceExcel = () => {
    ...
    const sheetRows = [
      ...
      [],
      headers,
      ...traceRows.map((row) => [row.lot, row.createdAt, row.customer, row.product, row.productGroup, row.formula, row.quantity, row.machine, row.chemicalActor, row.solidActor, row.mixingActor, row.qcActor, row.packingActor, row.warehouseActor, row.stage, row.status, row.latestTime]),
    ]
    const book = XLSX.utils.book_new()
...
function MachinePerformanceReportPage({ data }) {
  ...
  const rows = machines
    ...
    return {
      ...
      utilizationPercent,
      lastOrder: lastOrder ? getOrderLotCode(lastOrder) : '-',
    }
  })
...
function LegacyProductionAssignmentPage({ data, setData, user, permissions = [] }) {
  ...
  const teamStageOptions = {
    TP1: ['Phối trộn', 'Đóng gói', FINISHED_GOODS_LEGACY_STAGE],
    TP2: ['Phối trộn', 'Đóng gói', FINISHED_GOODS_LEGACY_STAGE],
    TH: ['Cân hóa'],
    TC: ['Cân rắn'],
  ...
  const weeklyRoleOptions = {
    TP1: [
      ...
      { value: 'lower', label: 'Tổ dưới', weeklyRole: 'Tổ dưới', operationPosition: 'Tổ dưới', stage: 'Đóng gói', processStages: ['Đóng gói', FINISHED_GOODS_LEGACY_STAGE] },
    ],
    TP2: [
      { value: 'upper', label: 'Tổ trên', weeklyRole: 'Tổ trên', operationPosition: 'Tổ trên', stage: 'Phối trộn', processStages: ['Phối trộn'] },
      { value: 'lower', label: 'Tổ dưới', weeklyRole: 'Tổ dưới', operationPosition: 'Tổ dưới', stage: 'Đóng gói', processStages: ['Đóng gói', FINISHED_GOODS_LEGACY_STAGE] },
...
const pageMeta = {
  ...
  'finished-goods': ['Thành phẩm', 'Ghi nhận thành phẩm và hoàn thành lệnh'],
...
function App() {
  const [data, setData] = useState(() => {
    ...
    const supplementalWeighing = nonEmptyArray(loadStored(SUPPLEMENTAL_WEIGHING_KEY, null), savedWithoutScaleTools.supplementalWeighing)
    ...
    const materialCatalog = deriveMaterialCatalog({
      formulas,
      rawMaterials,
      materialCatalog: nonEmptyArray(loadStored(MATERIAL_CATALOG_KEY, null), savedWithoutScaleTools.materialCatalog),
    })
    const storedOrders = loadStored(PRODUCTION_ORDERS_KEY, null)
    const orderSource = nonEmptyArray(storedOrders, savedWithoutScaleTools.productionOrders, savedWithoutScaleTools.orders, seed.orders)
    ...
    const baseData = {
      ...
      stockTransactions: savedWithoutScaleTools.stockTransactions || [],
      mixingMachines: normalizeMixingMachines(savedWithoutScaleTools.mixingMachines),
      productionAssignments: normalizeProductionAssignmentsData(savedWithoutScaleTools.productionAssignments || seed.productionAssignments || []),
      productionLogs,
    }
  ...
  const normalizeRequestedPage = (pageId) => {
    if (pageId === 'formulas') return 'master-products'
    if (pageId === 'raw-materials') return 'raw-material-lot-update'
    return pageId
  }
  const initialPage = normalizeRequestedPage(requestedPage)
  ...
  const changePage = (nextPage) => {
    const normalizedPage = normalizeRequestedPage(nextPage)
